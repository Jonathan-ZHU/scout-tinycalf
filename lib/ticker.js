...
updateOkcoin.on('start', function( dt ) {
  ...
  okcoin.tickerApi({symbol:symbol},function(err, ret){
    ...
    if(ret && ret.ticker && ret.ticker.sell) {
...
var klineinfo = null;
exports.getBTCKline = function (callback) {
  okcoin.klineDataApi({symbol:'btc_usd',type:'1hour'},function(err,ret){
    callback(err,ret);
  });
}
exports.getLTCKline = function (callback) {
  okcoin.klineDataApi({symbol:'ltc_usd',type:'1hour'},function(err,ret){
    callback(err,ret);
  });
}
exports.getETHKline = function (callback) {
  okcoin.klineDataApi({symbol:'eth_usd',type:'1hour'},function(err,ret){
    callback(err,ret);
  });
}
...
updateCoinbase.on('start', function( dt ) {
  ...
  coinbase.getSellPrice({'currencyPair': symbol},function(err, ret){
    ...
    if(ret && ret.data && ret.data.amount) {
