...
emitter.on('updateOkcoin', function( lt, dt ) {
  var symbol = null;
  switch(dt) {

  }
  okcoin.tickerApi({symbol:'btc_usd'},function(err, ret){
    if (err) {
      console.log(err);
      return;
    }
    ...
    emitter.emit('updateOkcoin', lt, dt );
  });
});
...
//update bitfinex CNY_BTC
var updateBitfinexCNYBTC = function (err,ret) {
  if (err) {
    console.log(err);
    return;
  }
  if(ret!=undefined) {
    ...
    num = fix(num);
    CNY_BTC.bitfinex = num;
  }
  bitfinex.ticker("btcusd",updateBitfinexCNYBTC);
};
updateBitfinexCNYBTC(null,null);

//update bitfinex CNY_LTC
var updateBitfinexCNYLTC = function (err,ret) {
  if (err) {
    console.log(err);
    return;
  }
  if(ret!=undefined) {
    var num = ret.last_price * rate.getRate();
    num = fix(num);
    CNY_LTC.bitfinex = num;
  }
  bitfinex.ticker("ltcusd",updateBitfinexCNYLTC);
};
updateBitfinexCNYLTC(null,null);

//update bitfinex CNY_ETH
var updateBitfinexCNYETH = function (err,ret) {
  if (err) {
    console.log(err);
    return;
  }
  if(ret!=undefined) {
    var num = ret.last_price * rate.getRate();
    num = fix(num);
    CNY_ETH.bitfinex = num;
  }
  bitfinex.ticker("ethusd",updateBitfinexCNYETH);
};
updateBitfinexCNYETH(null,null);
